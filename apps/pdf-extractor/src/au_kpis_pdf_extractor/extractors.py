...
class DeterministicExtractor:
    ...
    def extract(self, pdf_path: Path, artifact_key: str) -> ExtractionResponse:
        tables = self._camelot_tables(pdf_path)
        if not tables:
            tables = self._pdfplumber_tables(pdf_path)
    ...
    def _pdfplumber_tables(self, pdf_path: Path) -> list[TableCandidate]:
    ...
    def _camelot_tables(self, pdf_path: Path) -> list[TableCandidate]:
        try:
        ...
        pages = "all" if self.max_pages is None else f"1-{self.max_pages}"
