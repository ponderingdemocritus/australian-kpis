...
GitHub issue is the operational signal for actionable source review. Unresolved
register statuses such as `manual_pending`, `visible_unscored`, and
`coverage_gap` remain manual-review findings even when their live source checks
pass, so they keep the singleton issue open until reviewed. Report status
`error` fails the workflow after artifacts are uploaded because the audit could
not reliably verify sources.
