...
fn issue_37_benchmark_contract_is_wired() {
    ...
    assert!(
        merge_workflow.contains("merge_group:")
            ...
            && merge_workflow.contains("merge_queue: true"),
        "merge workflow should call the full PR flow for merge queue batches"
    );
    assert!(
...
fn issue_38_k6_smoke_contract_is_wired() {
    ...
    assert!(
        pr_workflow.contains("AU_KPIS_STAGING_BASE_URL")
            ...
            && merge_workflow.contains("merge_queue: true"),
        "merge-queue smoke flow should target the configured staging API through reusable PR flow inputs"
    );
...
fn issue_39_schemathesis_contract_is_wired() {
    ...
    assert!(
        merge_workflow.contains("merge_group:")
            && merge_workflow.contains("- full-pr-flow")
...
fn issue_62_full_load_test_contract_is_wired() {
    ...
    for expected in [
        ...
        "AU_KPIS_FULL_LOAD_DURATION || '30m'",
        "workload(0.45, 'warm-single-series'",
        "workload(0.15, 'cold-range-json'",
        "workload(0.1, 'rollups'",
        "workload(0.1, 'catalog-source-search'",
        "workload(0.1, 'aps-latest'",
        "workload(0.05, 'aps-history'",
        "workload(0.05, 'validation-rate-limit'",
