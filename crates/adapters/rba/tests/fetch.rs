...
const CSV_FIXTURE: &[u8] = include_bytes!("fixtures/f1_money_market_daily.csv");
...
async fn serve_artifact_once(body: &'static [u8]) -> (String, String) {
    ...
    tokio::spawn(async move {
        ...
        assert!(request.starts_with("GET /statistics/tables/csv/f01d.csv HTTP/1.1"));
        assert!(
            request
                .to_ascii_lowercase()
    ...
    let base_url = format!("http://{addr}/statistics/tables/");
    let source_url = format!("{base_url}csv/f01d.csv");
    (base_url, source_url)
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn fetch_persists_raw_rba_csv_artifact_with_response_headers() {
    let (index_url, source_url) = serve_artifact_once(CSV_FIXTURE).await;
    let adapter = RbaAdapter::builder().index_url(index_url).build();
    let table = RbaAdapter::parse_statistical_tables_index(&format!(
        r#"<a href="{source_url}" data-updated="2026-05-20">F1 - Data</a>"#
    ))
    .expect("parse table link")
    .into_iter()
    .next()
    .expect("table discovered");
    let job = RbaAdapter::current_jobs_with_started_at(
        &[table],
    ...
    let artifact = adapter
        .fetch(job.clone(), &ctx)
        .await
        .expect("fetch RBA CSV");

    assert_eq!(artifact.source_id.as_str(), "rba");
    assert_eq!(artifact.source_url, job.source_url);
    assert_eq!(artifact.content_type, "text/csv");
    assert_eq!(artifact.response_headers["x-rba-fixture"], vec!["f1"]);
