...
async fn snapshot_rows(artifact: ArtifactRef, blob_store: BlobStore) -> Vec<SnapshotRow> {
    ...
    .with_expected_dataflow(
        DataflowId::new("pc.productivity_bulletin").unwrap(),
        BTreeMap::new(),
    );
    let rows = adapter
        .parse(artifact, &ctx)
...
#[tokio::test]
async fn parses_productivity_bulletin_fixture() {
    ...
    assert!(
        rows.iter()
            .any(|row| row.measure_id == "market_sector_growth"
    ...
    assert!(
        rows.iter().any(
            |row| row.measure_id == "multifactor_productivity_growth" && row.value == Some(0.4)
        )
    );
...
async fn parse_rejects_mismatched_pc_source_and_dataflow() {
    ...
    assert!(
        err.to_string()
            .contains("PC parse received artifact for source")
    );
}
...
async fn parse_rejects_expected_dataflow_mismatch() {
    ...
    .with_expected_dataflow(DataflowId::new("pc.other").unwrap(), BTreeMap::new());
...
async fn parse_rejects_artifact_id_storage_key_mismatch() {
    ...
    let actual_id = blob_store
        .put_artifact(Bytes::from_static(PRODUCTIVITY_CSV))
        .await
        .expect("store fixture artifact");
    let wrong_id = ArtifactId::of_content(b"different PC productivity artifact");
    ...
    let artifact = ArtifactRef {
        ...
        size_bytes: PRODUCTIVITY_CSV.len() as u64,
        fetched_at: Utc.with_ymd_and_hms(2026, 6, 22, 0, 0, 0).unwrap(),
    };
    let adapter = PcAdapter::default();
    let http = AdapterHttpClient::new(adapter.manifest().rate_limit);
