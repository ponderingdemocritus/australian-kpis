...
<!doctype html>
<main>
  <h1>State planning throughput publications</h1>
  <a href="https://www.planning.nsw.gov.au/data/nsw-da-processing-2026.csv" data-dataflow="state_planning.nsw_da_processing" data-updated="2026-06-12">
    NSW DA processing dashboard export 2026
  </a>
  <a href="https://www.planning.vic.gov.au/data/vic-permit-activity-2026.csv" data-dataflow="state_planning.vic_permit_activity" data-updated="2026-06-10">
    Victoria planning permit activity export 2026
  </a>
...
async fn serve_index_once(body: &'static str) -> Option<String> {
    ...
    tokio::spawn(async move {
        ...
        assert!(request.starts_with("GET /planning HTTP/1.1"), "{request}");
        assert!(
            request
                .to_ascii_lowercase()
    ...
    Some(format!("http://{addr}/planning"))
}

#[test]
fn parse_index_discovers_state_planning_csv_artifacts() {
    let publications =
        StatePlanningAdapter::parse_publications(INDEX_FIXTURE).expect("parse index fixture");
...
fn discoverable_jobs_apply_revisions_and_source_metadata() {
    ...
    let known_revisions = BTreeMap::from([(
        "STATE_PLANNING:vic-permit-activity-2026".to_string(),
        UpstreamRevision::new("2026-06-10", Some("2026-06-10")),
    )]);
    let jobs = StatePlanningAdapter::discoverable_jobs_with_started_at(
        &current,
        &known_revisions,
...
async fn discover_honours_requested_dataflow_scope() {
    ...
    let adapter = StatePlanningAdapter::builder()
        .index_url(&index_url)
        .build();
    let http = AdapterHttpClient::new(adapter.manifest().rate_limit);
    let ctx = DiscoveryCtx::new(http, Utc.with_ymd_and_hms(2026, 6, 22, 0, 0, 0).unwrap())
        .with_requested_dataflow_id(
            au_kpis_domain::DataflowId::new("state_planning.vic_permit_activity").unwrap(),
        );

    let jobs = adapter
        .discover(&ctx)
        .await
        .expect("discover state planning publications");
...
#[test]
fn manifest_declares_state_planning_dataflows() {
    let adapter = StatePlanningAdapter::default();
    let manifest = adapter.manifest();

    assert_eq!(manifest.source_id.as_str(), "state-planning");
    ...
    assert_eq!(dataflows[0].id.as_str(), "state_planning.nsw_da_processing");
    assert_eq!(
        dataflows[0]
            .dimensions
            .iter()
            ...
            .collect::<Vec<_>>(),
        vec!["jurisdiction", "council", "development_type", "metric"]
    );
    assert_eq!(
    ...
    assert_eq!(
        dataflows[1]
            ...
            .map(au_kpis_domain::DimensionId::as_str)
            .collect::<Vec<_>>(),
        vec!["jurisdiction", "permit_type", "metric"]
    );
}
