...
fn ai_job(id: &str, dataflow_id: &str, source_url: String) -> DiscoveredJob {
    DiscoveredJob {
        id: id.into(),
...
async fn serve_throttle_once(expected_path: &'static str) -> Option<String> {
    ...
    tokio::spawn(async move {
        ...
        stream
            .write_all(
                b"HTTP/1.1 429 Too Many Requests\r\nretry-after: 23\r\nx-request-id: ai-readiness-throttle\r\ncontent-length: 0\r\n\r\n",
            )
            .await
...
async fn fetch_preserves_retry_after_on_oxford_throttle() {
    assert_fetch_throttle_for(
        "ai-readiness:gari-australia-2026",
        "oxford.gari",
        "/ai/gari-australia-2026.csv",
    )
    .await;
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn fetch_preserves_retry_after_on_naic_throttle() {
    assert_fetch_throttle_for(
        "ai-readiness:naic-ai-adoption-tracker-2026",
        "naic.ai_adoption_tracker",
        "/ai/naic-ai-adoption-tracker-2026.csv",
    )
    .await;
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn fetch_preserves_retry_after_on_abs_ai_rd_throttle() {
    assert_fetch_throttle_for(
        "ai-readiness:abs-ai-rd-2026",
        "abs.ai_rd",
        "/ai/abs-ai-rd-2026.csv",
    )
    .await;
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn fetch_preserves_retry_after_on_home_affairs_throttle() {
    assert_fetch_throttle_for(
        "ai-readiness:home-affairs-skillselect-talent-proxy-2026",
        "home_affairs.skillselect_talent_proxy",
        "/ai/home-affairs-skillselect-talent-proxy-2026.csv",
    )
    .await;
}
...
async fn assert_fetch_throttle_for(id: &str, dataflow_id: &str, expected_path: &'static str) {
    ...
    let err = adapter
        .fetch(
            ai_job(id, dataflow_id, source_url),
            &FetchCtx::new(
    ...
    match err {
        AdapterError::UpstreamStatus {
            ...
            assert_eq!(response_headers["x-request-id"], ["ai-readiness-throttle"]);
        }
